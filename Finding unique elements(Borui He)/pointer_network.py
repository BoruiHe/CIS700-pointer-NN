...
class PointerNet(nn.Module):
    ...
    def __init__(self, input_size, embed_size, hidden_size, weight_size, output_size, num_layers, dict_size):
        ...
        self.W1 = nn.Sequential(nn.Linear(self.hidden_size,self.weight_size),
                                nn.ReLU()) # bias  is False
        self.W2 = nn.Sequential(nn.Linear(self.hidden_size,self.weight_size),
                                nn.ReLU())
        self.v  = nn.Sequential(nn.Linear(self.weight_size,1),
                                nn.Dropout(0.5)) 
    ...
    def forward(self,input):
        ...
        out_e = out_e.transpose(0,1) #([8,5,64]) should be [5,8,64]

        pred = []
        for i in range(self.output_size):
            dh,dc = self.Decoder(out_0,(dh,dc)) #dh size:[samples_num,hidden_size]
            u = torch.tanh(self.W1(out_e) + self.W2(dh)) # [input_size,samples_num,weight] + [samples_num,weight]
            out_s = self.v(u).squeeze()  ## size: [input_size,samples_num]
            out_s = F.softmax(out_s.transpose(0,1),1)   # size: [samples_num,input_size]
