...
class Running:
    ...
    def __init__(self,input_size=8,hidden_size=256,weight_size=128,samples_num=400):
    ...
    def importdata(self):
        ...
        data = Dataset(self.samples_num,self.input_size)
        ...
        data2 = Dataset(int(self.samples_num*0.2),self.input_size-3)
        xx_list,yy_list = data2.generatedata()
        mask = xx_list[0][-1]
        mask_pos = len(xx_list[0])-1
        for i in range(len(xx_list)):
            xx_list[i].append(mask)
            xx_list[i].append(mask)
            yy_list[i].append(mask_pos)
            yy_list[i].append(mask_pos)
...
def main():
    ...
    for hid in hidden_size:
        for seq in seq_length:
            ...
            newnet = Running(input_size=seq,hidden_size=hid)
            newnet.importdata()
            print("--------Training---------")
            newnet.train(batch)
            print("  \n")
